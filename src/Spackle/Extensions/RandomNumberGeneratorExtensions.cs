...
public static class RandomNumberGeneratorExtensions
{
	...
	extension(RandomNumberGenerator self)
	{
		/// <summary>
		...
		public BigInteger GetBigInteger(ulong numberOfDigits)
		{
			...
			if (numberOfDigits < 10)
			{
				...
				return new BigInteger(self.Next(lowerBound, lowerBound * 10));
			}
			else
			{
				...
				return delta == 0 ?
					...
					delta > 0 ?
						...
						(number * BigInteger.Pow(new BigInteger(10), -1 * delta)) + self.Next(10);
			}
		}
		...
		public BigInteger GetBigIntegerWithRange(BigInteger max)
		{
			...
			var numberOfDigits = (uint)self.Next(maxNumberOfDigits);

			numberOfDigits = numberOfDigits == 0 ? 1 : numberOfDigits;

			var result = self.GetBigInteger(numberOfDigits);
		...
		public BigInteger GetBigIntegerWithRange(BigInteger min, BigInteger max)
		{
			...
			var delta = self.GetBigIntegerWithRange(difference);
			return min + delta;
		}
		...
		/// the longer it will take for <see cref="GetByteValues(RandomNumberGenerator, uint, ValueGeneration)"/> to produce a unique random set of values.
		/// </remarks>
		public byte[] GetByteValues(uint numberOfElements, ValueGeneration values)
		{
			if (values == ValueGeneration.UniqueValuesOnly && numberOfElements > byte.MaxValue)
			{
				throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
					RandomNumberGeneratorExtensions.ErrorTooManyUniqueElements, byte.MaxValue), nameof(numberOfElements));
			}
		...
		/// the longer it will take for <see cref="GetInt32Values(RandomNumberGenerator, uint, ValueGeneration)"/> to produce a unique random set of values.
		/// </remarks>
		public int[] GetInt32Values(uint numberOfElements, ValueGeneration values) =>
			self.GetInt32Values(numberOfElements, 0..int.MaxValue, values);
		...
		/// the longer it will take for <see cref="GetInt32Values(RandomNumberGenerator, uint, Range, ValueGeneration)"/> to produce a unique random set of values.
		/// </remarks>
		public int[] GetInt32Values(uint numberOfElements, Range range, ValueGeneration values)
		{
			...
			{
				throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
					RandomNumberGeneratorExtensions.ErrorTooManyUniqueElements, int.MaxValue), nameof(numberOfElements));
			}
			...
			if (values == ValueGeneration.DuplicatesAllowed)
			{
				for (var i = 0; i < numberOfElements; i++)
				{
					elements[i] = self.Next(range.Start.Value, range.End.Value);
				}
			}
			else
			{
				...
				while (createdElementsCount < numberOfElements)
				{
					var value = self.Next(range.Start.Value, range.End.Value);
		...
		public double NextDouble() =>
			self.Next(int.MaxValue) * MaxInt32Inverse;
	}
}